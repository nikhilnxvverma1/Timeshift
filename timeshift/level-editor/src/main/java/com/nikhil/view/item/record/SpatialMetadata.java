...
/**
 * Spatial metadata provides a motion path to be associated with the movement of an item.
 * This class is dedicated to TRANSLATION property only.(Although that's not enforced yet)
 * Created by NikhilVerma on 11/11/15.
 */
public class SpatialMetadata extends PropertyMetadata {
    ...
    public Node getValueNode() {
        if (tag == MetadataTag.TRANSLATION) {
            return getTranslationValueNode();
        } else if (tag == MetadataTag.ANCHOR_POINT) {
    ...
    public void pushManualKeyframeCommand(double time) {
        SpatialKeyframeView newKeyframe = createNewKeyframe(spatialKeyframeChangeNode.getCurrentPoint(), time);
    ...
    protected KeyframeView createNewKeyframe(double time) {
        return createNewKeyframe(spatialKeyframeChangeNode.getCurrentPoint(),time);
    }
    ...
    @Override
    public SpatialKeyframeChangeNode getKeyframeChangeNode() {
